...
# 10-Fold Cross validation with GridSerach (to find best parameters for Decision Tree)
...
# 10-Fold Cross validation with GridSerach (to find best parameters for KNN)
ada_params={'n_estimators':np.arange(1,26),'learning_rate':[0.5,0.6,0.7,0.8,0.9,1]}

ada_cv=GridSearchCV(AdaBoostClassifier(random_state=0), ada_params, cv=10)


result=[]
#DecisionTree
result.append(result_generator(tree_cv,'Decision Tree CV',X_train,X_test,y_train,y_test))

#KNN
result.append(result_generator(knn_cv,'KNN CV',X_train,X_test,y_train,y_test))

#AdaBoost

result.append(result_generator(ada_cv,'AdaBoost CV',X_train,X_test,y_train,y_test))
